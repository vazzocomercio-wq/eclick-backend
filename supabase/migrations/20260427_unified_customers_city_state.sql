-- Cidade/UF em unified_customers — coluna na tabela /clientes
-- Origem: orders.billing_address->'city'->>'name' / 'state'->>'name'
-- (populado pelo fluxo 2-passos da ML billing_info).
...
-- 2. Backfill — pega city/state do billing_address mais recente do comprador
UPDATE unified_customers uc
SET city  = COALESCE(uc.city,  latest.billing_address->'city'->>'name'),
...
-- 3. Trigger sync_buyer_to_unified atualizado pra propagar city/state
