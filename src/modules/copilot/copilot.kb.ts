...
**Preço de atacado (B2B)**: campo "Desconto p/ preço de atacado (%)" calcula \`precoAtacado = precoPromo × (1 − descontoAtacado)\` — é o preço por quantidade do ML (compradores B2B). Por regra do ML o preço de atacado tem que ser menor que o de promoção, então o desconto incide sobre o preço de promoção. O painel mostra o valor e a margem nesse preço (vermelho se ficar negativa). API ML: \`POST /items/{id}/prices/standard/quantity\` com \`conditions.min_purchase_unit\` + \`context_restrictions: [channel_marketplace, user_type_business]\`, máx 5 faixas, só para sellers com a tag \`business\`. É um passo PÓS-publicação (precisa do item_id).
